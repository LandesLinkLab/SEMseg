function [fitobj, gof, output] = fitstadium( pts , cofmass , wgts)
...
thetas = atan2d(dispvect(:,2),dispvect(:,1));

aFittype = fittype('stadiumFcn(x, L, R, offset_angle, true_center_x,true_center_y)');
...
offset_angle_init = atand(coeff(2,1)/coeff(1,1));
offset_angle_init = wrapTo360(offset_angle_init);
if offset_angle_init > 180
    offset_angle_init = offset_angle_init - 180;
end

% Calculate parameter limits and initailization
L_opt = [1,4*max(rhos),2*mean(rhos)];
R_opt = [1,4*max(rhos),1*mean(rhos)];
offset_angle_opt = [0,180,offset_angle_init];
